...
# Initializing
	def __init__(self,k,d):
		...
		self.Sigma = np.tile(np.identity(d),(k,1)).reshape(k,d,d)
...
# Calculating mu, sigma and probability of each class in loop
		for q in range(self.classes.shape[0]):
			...
			self.Sigma[q] = np.matmul(np.transpose(X_c - self.Mean[q]),X_c - self.Mean[q])/X_c.shape[0]
			if diag ==True:
				self.Sigma[q] = self.Sigma[q]*np.identity(X_c.shape[1])
			if np.linalg.det(self.Sigma[q])==0:
				self.Sigma[q] = self.Sigma[q] + 0.01*np.identity(X_c.shape[1])
...
# Finding Descriminant for each class and returning the index of maximum value and retuning the class from class array.
		for j in range(X.shape[0]):
			...
			for i in range(self.classes.shape[0]):
				row = X[j]-self.Mean[i]
				row = np.array([row])
				P[i]= math.log(self.Prob[i]) - math.log(np.linalg.det(self.Sigma[i]))/2 - np.matmul(np.matmul(row,np.linalg.inv(self.Sigma[i])),row.T)[0,0]/2
			max_prob = np.amax(P)
